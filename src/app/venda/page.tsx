...
type PedidoCatalogo = {
  ...
  total: number;
  created_at: string;
  catalogo_carrinho_itens: PedidoCatalogoItem[];
};

type PedidoCatalogoItem = {
  id: string;
  descricao: string;
  cor: string | null;
  tamanho: string;
...
async function getCroppedImg(imageSrc: string, pixelCrop: AreaCrop): Promise<Blob> {
  ...
  return new Promise((resolve) => {
    canvas.toBlob((blob) => { if (blob) resolve(blob); }, 'image/jpeg', 1);
  });
}
...
function VendaPageInner() {
  ...
  // Catálogo
  const [abaDrafts, setAbaDrafts] = useState<'rascunhos' | 'catalogo'>('rascunhos');
  ...
  const [catalogoExpanded, setCatalogoExpanded] = useState<Record<string, boolean>>({});
  ...
  // Computa a URL pública da thumb a partir da foto_url original.
  // As thumbs ficam em produtos/thumbs/ (geradas no upload ou retroativamente).
  const getThumbUrl = useCallback((fotoUrl: string | null): string | null => {
    ...
    if (mostrarScanner) {
      ...
      const t = setTimeout(() => {
        ...
        html5QrCode.start(
          ...
          (decodedText) => { handleScanSucesso(decodedText); fecharScanner(); },
          () => {}
        ).catch((err) => { console.error('Erro Câmera:', err); setMostrarScanner(false); });
      }, 300);
  ...
  async function fetchDrafts() {
    const { data, error } = await supabase
      ...
      .limit(30);
    if (error) { console.log('Erro ao listar drafts:', error.message); return; }
  ...
  async function fetchPedidosCatalogo() {
    setLoadingCatalogo(true);
    try {
      const { data, error } = await supabase
        .from('catalogo_carrinhos')
        .select('id, token, nome_cliente, total, created_at, catalogo_carrinho_itens(id, descricao, cor, tamanho, preco, quantidade, foto_url)')
        .eq('status', 'pendente')
        .order('created_at', { ascending: false })
        .limit(50);
      if (error) throw new Error(error.message);
      setPedidosCatalogo((data ?? []) as any);
    } catch (e: any) {
      alert(`Erro ao buscar pedidos: ${e?.message ?? e}`);
    } finally {
      setLoadingCatalogo(false);
    }
  }

  async function importarPedidoCatalogo(carrinhoId: string) {
    if (!confirm('Importar este pedido como rascunho?')) return;
    setImportandoId(carrinhoId);
    try {
      const { data, error } = await supabase.rpc('importar_carrinho_catalogo', { p_carrinho_id: carrinhoId });
      if (error) throw new Error(error.message);
      await fetchDrafts();
      await fetchPedidosCatalogo();
      await carregarDraft(data as string);
  ...
  function buscarPorEAN(codigo: string) {
    ...
    for (const p of produtos) {
      ...
      if (estoqueEncontrado) { prepararAdicao(p, estoqueEncontrado); return true; }
    }
    return false;
  }
  ...
  function confirmarAdicao() {
    ...
    if (qtdNoCarrinho + 1 > est.quantidade) { alert(`Estoque insuficiente! Restam apenas ${est.quantidade}.`); return; }
    if (jaNoCarrinho) {
  ...
  function bloquearFechamentoSeZerados() {
    ...
    alert(`Existem itens no carrinho com ESTOQUE ZERADO.\n\nRemova os itens zerados antes de fechar a venda.\n\nItens:\n${lista}${itensZerados.length > 6 ? `\n• ... (+${itensZerados.length - 6})` : ''}`);
    return true;
  }
  ...
  function tituloPadraoDraft() {
    const d = new Date();
    return `Carrinho ${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
  }
  ...
  async function salvarDraft() {
    ...
    try {
      ...
      if (!draftId) {
        const { data: created, error: errCreate } = await supabase.from('venda_drafts').insert([{ titulo: tituloFinal }]).select('id').single();
      ...
      const itens = carrinho.map((i) => ({ draft_id: draftId, produto_id: i.produto_id, estoque_id: i.estoque_id, descricao: i.descricao, cor: i.cor, tamanho: i.tamanho, preco: i.preco, custo: i.custo, qtd: i.qtd, foto_url: i.foto, ean: i.ean }));
      ...
      if (draftId) setDraftItemsCache((prev) => { const cp = { ...prev }; delete cp[draftId!]; return cp; });
    } catch (e: any) {
  ...
  async function carregarDraft(draftId: string, draftTitulo?: string | null) {
    setLoading(true);
    try {
      const { data, error } = await supabase.from('venda_draft_itens').select('produto_id, estoque_id, descricao, cor, tamanho, preco, custo, qtd, foto_url, ean').eq('draft_id', draftId);
      if (error) throw new Error(error.message);
      ...
      const novoCarrinho: ItemCarrinho[] = items.map((it) => {
        ...
        return { tempId: Math.random().toString(36), produto_id: it.produto_id, estoque_id: it.estoque_id, descricao: it.descricao, cor: it.cor ?? '', tamanho: it.tamanho ?? '', preco: Number(it.preco) || 0, custo: Number(it.custo) || 0, qtd: Math.max(1, Number(it.qtd) || 1), maxEstoque: est?.quantidade ?? 0, foto: it.foto_url, ean: it.ean };
      });
      ...
      if (zerados.length > 0) {
        const lista = zerados.slice(0, 6).map((i) => `• ${i.descricao} (${i.cor} • ${i.tamanho})`).join('\n');
        alert(`Carrinho carregado, mas existem itens com ESTOQUE ZERADO.\n\nEles foram mantidos no carrinho para você visualizar, porém você precisa removê-los antes de fechar a venda.\n\nItens:\n${lista}${zerados.length > 6 ? `\n• ... (+${zerados.length - 6})` : ''}`);
      }
    } catch (e: any) {
  ...
  async function excluirDraft(draftId: string) {
    ...
    try {
      ...
      if (error) throw new Error(error.message);
      if (draftAtualId === draftId) { setDraftAtualId(null); setDraftAtualTitulo(''); }
      await fetchDrafts();
      setDraftItemsCache((prev) => { const cp = { ...prev }; delete cp[draftId]; return cp; });
      setDraftExpanded((prev) => { const cp = { ...prev }; delete cp[draftId]; return cp; });
    } catch (e: any) {
      alert(`Erro ao excluir: ${e?.message ?? e}`);
    } finally { setLoading(false); }
  }

  async function excluirPedidoCatalogo(carrinhoId: string) {
    if (!confirm('Excluir este pedido do catálogo? Esta ação não pode ser desfeita.')) return;
    try {
      const { error } = await supabase.from('catalogo_carrinhos').update({ status: 'expirado' }).eq('id', carrinhoId);
      if (error) throw new Error(error.message);
      await fetchPedidosCatalogo();
    } catch (e: any) {
      alert(`Erro ao excluir: ${e?.message ?? e}`);
  ...
  function limparCarrinho() {
    ...
    const msg = carrinho.length === 0 && draftAtualId ? 'Limpar o rascunho atual (desvincular do carrinho) ?' : 'Limpar carrinho atual?';
  ...
  const handleDescontoInput = (valor: string, tipo: 'reais' | 'porcentagem' | 'final') => {
    ...
    if (tipo === 'final') {
      setPagamento((prev) => ({ ...prev, descontoTipo: 'reais', descontoValor: parseFloat((totalBruto - num).toFixed(2)) }));
    } else {
    ...
    <div className={`min-h-screen ${bgClass} text-slate-100 font-sans flex flex-col md:flex-row overflow-hidden relative`}>
      ...
      <div className={`relative z-10 flex-1 p-4 md:p-6 flex flex-col gap-4 h-[calc(100vh-80px)] md:h-screen overflow-y-auto ${abaMobile === 'carrinho' ? 'hidden md:flex' : 'flex'}`}>
        <header className="flex items-center gap-4">
          ...
          {isEditandoDraft && (
            <span className="ml-auto bg-orange-600 text-white text-[10px] px-3 py-1 rounded-full font-black uppercase tracking-widest border border-orange-300/30 shadow">MODO EDIÇÃO</span>
          )}
        </header>
          ...
          <div className="bg-orange-900/35 border border-orange-500/40 rounded-2xl p-4 shadow-[0_0_0_1px_rgba(0,0,0,0.2)]">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[10px] font-black uppercase tracking-widest text-orange-200">Você está editando um carrinho salvo</p>
                <p className="text-white font-black truncate">{draftAtualTitulo || 'Carrinho salvo'}</p>
                {temZerados && <p className="mt-2 text-[11px] font-bold text-red-200">Atenção: existem itens com <span className="font-black">ESTOQUE ZERADO</span>. Remova-os para fechar a venda.</p>}
              </div>
              <button onClick={limparCarrinho} className="shrink-0 text-[10px] font-black uppercase text-red-200 hover:text-white bg-red-950/30 border border-red-900/30 px-3 py-2 rounded-xl active:scale-95">Sair da edição</button>
            </div>
          </div>
        )}
            ...
            <div className="max-h-[55vh] overflow-y-auto">
              {produtosVisiveis.length === 0 ? (
                ...
                produtosVisiveis.map((p, idx) => {
                  ...
                  return (
                    <button key={p.id} onClick={() => handleSelecionarProduto(p)} className="w-full text-left p-4 hover:bg-slate-800 border-b border-slate-800/60 last:border-0 flex justify-between items-center transition-colors active:bg-slate-700 gap-4">
                      <div className="flex items-center gap-4 min-w-0">
                        <div className="w-12 h-12 rounded-xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center flex-shrink-0">
                          {thumbUrl ? <img src={thumbUrl} className="w-full h-full object-cover" alt="" loading={idx < 6 ? 'eager' : 'lazy'} decoding="async" /> : <span className="text-lg opacity-60">📷</span>}
                        </div>
                        <div className="min-w-0">
                          <p className="font-bold text-white text-sm uppercase truncate">{p.descricao}</p>
                          <p className="text-[10px] font-mono text-slate-400 truncate">{p.codigo_peca} | {p.cor}</p>
                        </div>
                      </div>
      ...
      <div className={`relative z-10 w-full md:w-[420px] md:border-l flex flex-col h-screen md:sticky md:top-0 shadow-2xl ${abaMobile === 'busca' ? 'hidden md:flex' : 'flex fixed inset-0'} ${isEditandoDraft ? 'border-orange-800/50' : 'border-slate-800'}`}>
        {isEditandoDraft ? (
          <>
            <div className="absolute inset-0 z-0 bg-gradient-to-br from-orange-950 via-amber-950 to-slate-950" />
            <div className="pointer-events-none absolute inset-0 z-0"><div className="absolute inset-0 opacity-[0.10]"></div></div>
          </>
        ) : (
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="p-6 bg-slate-950/95 backdrop-blur-md border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setAbaMobile('busca')} className="md:hidden bg-slate-800 p-2 rounded-full text-slate-300">←</button>
              ...
              {isEditandoDraft && <span className="ml-2 bg-orange-600 text-white text-[10px] px-3 py-1 rounded-full font-black uppercase tracking-widest border border-orange-300/30 shadow">MODO EDIÇÃO</span>}
            </div>
            <div className="flex items-center gap-2">
              <button disabled={loading} onClick={() => { setMostrarDrafts(true); setAbaDrafts('rascunhos'); fetchDrafts(); }} className="bg-slate-800 hover:bg-slate-700 text-white px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-700 active:scale-95" title="Carrinhos salvos">📂</button>
              <button disabled={loading || carrinho.length === 0} onClick={salvarDraft} className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-blue-400/30 active:scale-95 disabled:opacity-50" title={isEditandoDraft ? 'Atualizar carrinho salvo' : 'Salvar carrinho'}>💾</button>
            </div>
          </div>
          ...
          {temZerados && (
            <div className="px-4 pt-3">
              <div className="bg-red-950/25 border border-red-900/30 rounded-2xl p-3">
                <p className="text-[11px] font-bold text-red-200">Existem <span className="font-black">{itensZerados.length}</span> item(ns) com <span className="font-black">ESTOQUE ZERADO</span>. Remova-os antes de fechar a venda.</p>
              </div>
            </div>
          )}
                ...
                <div key={item.tempId} className={`bg-slate-900/90 p-3 rounded-2xl border flex gap-3 relative group transition-colors shadow-sm ${zerado ? 'border-red-900/60 hover:border-red-700' : 'border-slate-800 hover:border-slate-600'}`}>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {zerado ? <span className="text-[9px] font-black uppercase text-red-200 bg-red-950/30 border border-red-900/30 px-2 py-0.5 rounded-lg">estoque 0</span> : <span className="text-[9px] font-bold uppercase text-slate-400 bg-slate-950/40 border border-slate-800 px-2 py-0.5 rounded-lg">disp. {item.maxEstoque}</span>}
                    </div>
                  </div>
                  <div className="flex flex-col items-end justify-between py-1">
                    <div className="flex items-center gap-1 bg-slate-950 rounded-lg p-1 border border-slate-800">
                      ...
                      <button onClick={() => alterarQtd(item.tempId, 1)} disabled={disablePlus} className={`w-8 h-8 flex items-center justify-center rounded-lg text-lg font-bold transition active:scale-90 ${disablePlus ? 'bg-slate-800/60 text-slate-600 cursor-not-allowed' : 'bg-slate-800 hover:bg-green-500/20 hover:text-green-500 text-slate-400'}`}>+</button>
                    </div>
          ...
          <div className="p-6 bg-slate-950/95 backdrop-blur-md border-t border-slate-800 space-y-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button disabled={loading || carrinho.length === 0} onClick={salvarDraft} className="w-full bg-slate-800 hover:bg-slate-700 text-white font-black py-5 rounded-xl shadow-lg uppercase tracking-widest text-[10px] transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 h-16">{isEditandoDraft ? 'ATUALIZAR' : 'SALVAR'}</button>
              <button disabled={loading || carrinho.length === 0} onClick={abrirModalPagamento} className={`w-full text-white font-black py-5 rounded-xl shadow-lg uppercase tracking-widest text-[10px] transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 h-16 ${temZerados ? 'bg-red-700 hover:bg-red-600' : 'bg-gradient-to-r from-emerald-600 to-emerald-500 hover:brightness-110'}`}>{loading ? 'PROCESSANDO...' : temZerados ? 'REMOVA ITENS ZERADOS' : 'FECHAR VENDA (F2)'}</button>
            </div>
          </div>
        </div>
      </div>

      {/* MODAL: CARRINHOS (RASCUNHOS + CATÁLOGO) */}
      {mostrarDrafts && (
        <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="bg-slate-900 w-full max-w-2xl rounded-t-[2.5rem] md:rounded-[2.5rem] border-t md:border border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
            ...
            <div className="bg-slate-950 p-6 border-b border-slate-800 flex justify-between items-center shrink-0">
              <h2 className="text-xl font-black uppercase text-white tracking-widest">Carrinhos</h2>
              <button onClick={() => setMostrarDrafts(false)} className="w-10 h-10 rounded-full bg-slate-800 text-slate-400 hover:text-white font-bold transition active:scale-90">✕</button>
            </div>

            {/* ABAS */}
            <div className="flex border-b border-slate-800 shrink-0">
              <button
                onClick={() => setAbaDrafts('rascunhos')}
                className={`flex-1 py-4 text-[11px] font-black uppercase tracking-widest transition-colors ${abaDrafts === 'rascunhos' ? 'text-white border-b-2 border-pink-500 bg-slate-900' : 'text-slate-500 hover:text-slate-300 bg-slate-950'}`}
              >
                💾 Rascunhos{drafts.length > 0 && <span className="ml-1 bg-slate-700 text-slate-300 px-2 py-0.5 rounded-full text-[9px]">{drafts.length}</span>}
              </button>
              <button
                onClick={() => { setAbaDrafts('catalogo'); fetchPedidosCatalogo(); }}
                className={`flex-1 py-4 text-[11px] font-black uppercase tracking-widest transition-colors ${abaDrafts === 'catalogo' ? 'text-white border-b-2 border-pink-500 bg-slate-900' : 'text-slate-500 hover:text-slate-300 bg-slate-950'}`}
              >
                🛍️ Catálogo{pedidosCatalogo.length > 0 && <span className="ml-1 bg-pink-700 text-pink-200 px-2 py-0.5 rounded-full text-[9px]">{pedidosCatalogo.length}</span>}
              </button>
            </div>
            ...
            {abaDrafts === 'rascunhos' && (
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {drafts.length === 0 ? (
                  <div className="text-slate-500 text-sm font-bold uppercase tracking-widest text-center py-10">Nenhum carrinho salvo</div>
                ) : (
                  drafts.map((d) => {
                    ...
                    return (
                      <div key={d.id} className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
                        <div className="p-4 flex items-center justify-between gap-4">
                          <button className="min-w-0 text-left flex-1" onClick={async () => { const next = !isOpen; setDraftExpanded((prev) => ({ ...prev, [d.id]: next })); if (next) await fetchDraftItemsPreview(d.id); }}>
                            <p className="text-sm font-black text-white truncate">{d.titulo || 'Carrinho salvo'}</p>
                            <p className="text-[10px] font-mono text-slate-500">{new Date(d.updated_at || d.created_at).toLocaleString('pt-BR')}</p>
                            <div className="mt-2 flex items-center gap-2">
                              {draftAtualId === d.id && <span className="text-[10px] font-black uppercase text-emerald-400 bg-emerald-900/20 border border-emerald-900/30 px-2 py-1 rounded-lg">Ativo</span>}
                              <span className="text-[10px] font-black uppercase text-slate-300 bg-slate-900/40 border border-slate-800 px-2 py-1 rounded-lg">{isOpen ? 'Ocultar itens' : 'Ver itens'}</span>
                              {items.length > 0 && <span className="text-[10px] font-black uppercase text-slate-300 bg-slate-900/40 border border-slate-800 px-2 py-1 rounded-lg">{itensResumo} itens</span>}
                            </div>
                          </button>
                          <div className="flex items-center gap-2 shrink-0 pr-1">
                            <button disabled={loading} onClick={() => carregarDraft(d.id, d.titulo)} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95">Carregar</button>
                            <button disabled={loading} onClick={() => excluirDraft(d.id)} className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95">Excluir</button>
                          </div>
                        </div>
                        {isOpen && (
                          <div className="border-t border-slate-800 bg-slate-950/60 p-4">
                            {isLoadingItems ? (
                              <div className="text-slate-400 text-xs font-bold uppercase tracking-widest py-3">Carregando itens...</div>
                            ) : items.length === 0 ? (
                              <div className="text-slate-500 text-xs font-bold uppercase tracking-widest py-3">Nenhum item neste carrinho</div>
                            ) : (
                              <div className="space-y-2">
                                {items.slice(0, 10).map((it, idx) => (
                                  <div key={`${it.estoque_id}-${idx}`} className="flex items-center justify-between gap-3 bg-slate-900/40 border border-slate-800 rounded-xl px-3 py-2">
                                    <div className="min-w-0">
                                      <p className="text-[11px] font-black text-white truncate">{it.descricao}</p>
                                      <p className="text-[9px] font-bold text-slate-400 uppercase truncate">{(it.cor ?? '').trim()} • {(it.tamanho ?? '').trim()}</p>
                                    </div>
                                ...
                                {items.length > 10 && <div className="text-slate-400 text-[10px] font-bold uppercase tracking-widest pt-2">+{items.length - 10} item(ns)...</div>}
                              </div>
                            )}
                          </div>
                        )}
            ...
            {abaDrafts === 'catalogo' && (
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {loadingCatalogo ? (
                  <div className="text-slate-400 text-xs font-bold uppercase tracking-widest text-center py-10 animate-pulse">Carregando pedidos...</div>
                ) : pedidosCatalogo.length === 0 ? (
                  <div className="text-slate-500 text-sm font-bold uppercase tracking-widest text-center py-10">Nenhum pedido pendente</div>
                ) : (
                  pedidosCatalogo.map((pedido) => {
                    const isOpen = !!catalogoExpanded[pedido.id];
                    const isImportando = importandoId === pedido.id;
                    const totalItens = pedido.catalogo_carrinho_itens.reduce((acc, i) => acc + i.quantidade, 0);
                    return (
                      <div key={pedido.id} className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
                        <div className="p-4 flex items-center justify-between gap-4">
                          <button className="min-w-0 text-left flex-1" onClick={() => setCatalogoExpanded((prev) => ({ ...prev, [pedido.id]: !isOpen }))}>
                            <p className="text-sm font-black text-white truncate">{pedido.nome_cliente || 'Cliente sem nome'}</p>
                            <p className="text-[10px] font-mono text-slate-500">{new Date(pedido.created_at).toLocaleString('pt-BR')}</p>
                            {/* Token para validação */}
                            <p className="text-[9px] font-mono text-slate-600 truncate mt-0.5">#{pedido.token}</p>
                            <div className="mt-2 flex items-center gap-2 flex-wrap">
                              <span className="text-[10px] font-black text-emerald-400 bg-emerald-900/20 border border-emerald-900/30 px-2 py-1 rounded-lg">R$ {Number(pedido.total).toFixed(2).replace('.', ',')}</span>
                              <span className="text-[10px] font-black text-slate-300 bg-slate-900/40 border border-slate-800 px-2 py-1 rounded-lg">{totalItens} item(ns)</span>
                              <span className="text-[10px] font-black text-slate-400 bg-slate-900/40 border border-slate-800 px-2 py-1 rounded-lg">{isOpen ? 'Ocultar' : 'Ver itens'}</span>
                            </div>
                          </button>
                          <div className="flex flex-col gap-2 shrink-0">
                            <button disabled={isImportando} onClick={() => importarPedidoCatalogo(pedido.id)} className="bg-pink-600 hover:bg-pink-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 disabled:opacity-50">
                              {isImportando ? '...' : 'Importar'}
                            </button>
                            <button onClick={() => excluirPedidoCatalogo(pedido.id)} className="bg-slate-800 hover:bg-red-900/40 text-slate-400 hover:text-red-300 border border-slate-700 hover:border-red-900/50 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-colors">
                              Excluir
                            </button>
                          </div>
                        </div>
                        {isOpen && (
                          <div className="border-t border-slate-800 bg-slate-950/60 p-4 space-y-2">
                            {pedido.catalogo_carrinho_itens.map((item, idx) => (
                              <div key={`${item.id}-${idx}`} className="flex items-center gap-3 bg-slate-900/40 border border-slate-800 rounded-xl px-3 py-2">
                                {/* Foto miniatura */}
                                <div className="w-10 h-10 rounded-lg bg-slate-800 border border-slate-700 overflow-hidden flex-shrink-0 flex items-center justify-center">
                                  {item.foto_url ? (
                                    <img src={item.foto_url} className="w-full h-full object-cover" alt="" loading="lazy" decoding="async" />
                                  ) : (
                                    <span className="text-base opacity-40">📷</span>
                                  )}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-[11px] font-black text-white truncate">{item.descricao}</p>
                                  <p className="text-[9px] font-bold text-slate-400 uppercase">{item.tamanho}{item.cor ? ` • ${item.cor}` : ''}</p>
                                </div>
                                <div className="shrink-0 text-right">
                                  <p className="text-[10px] font-black text-white">x{item.quantidade}</p>
                                  <p className="text-[10px] font-mono text-slate-400">R$ {Number(item.preco).toFixed(2).replace('.', ',')}</p>
                                </div>
                              </div>
                            ))}
                          </div>
            ...
            <div className="p-6 bg-slate-950 border-t border-slate-800 shrink-0 flex gap-3">
              <button onClick={() => abaDrafts === 'rascunhos' ? fetchDrafts() : fetchPedidosCatalogo()} className="flex-1 bg-slate-800 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-[10px] hover:bg-slate-700 active:scale-95">Atualizar</button>
              <button onClick={() => setMostrarDrafts(false)} className="flex-1 bg-pink-600 text-white font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-[10px] hover:bg-pink-500 active:scale-95">Fechar</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="flex justify-between items-start mb-6 shrink-0 gap-4">
              <div className="flex items-start gap-4 min-w-0">
                <div className="w-16 h-16 rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center shrink-0">
                  {modalSelecao.foto_url && getThumbUrl(modalSelecao.foto_url) ? <img src={getThumbUrl(modalSelecao.foto_url)!} className="w-full h-full object-cover" alt="" loading="eager" decoding="async" /> : <span className="text-2xl opacity-50">📷</span>}
                </div>
                <div className="min-w-0">
              ...
              <button onClick={() => setModalSelecao(null)} className="bg-slate-800 w-10 h-10 rounded-full text-slate-400 hover:text-white font-bold text-xl active:scale-90 shrink-0">✕</button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-4">
                  <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center gap-3">
                    ...
                    <input type="number" inputMode="decimal" placeholder="0" className="bg-transparent w-full text-lg font-bold text-white outline-none" value={pagamento.descontoTipo === 'porcentagem' ? pagamento.descontoValor : ''} onChange={(e) => handleDescontoInput(e.target.value, 'porcentagem')} />
                    <span className="text-xs text-slate-600 font-bold uppercase">Desc. %</span>
                  </div>
                  <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex items-center gap-3">
                    <span className="text-blue-500 font-bold">R$</span>
                    <input type="number" inputMode="decimal" placeholder="0,00" className="bg-transparent w-full text-lg font-bold text-white outline-none" value={pagamento.descontoTipo === 'reais' ? pagamento.descontoValor : ''} onChange={(e) => handleDescontoInput(e.target.value, 'reais')} />
                ...
                <div className="bg-slate-950 p-6 rounded-3xl border border-slate-800 text-center relative group">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <input type="number" inputMode="decimal" className="bg-transparent text-5xl font-black text-white outline-none text-center w-64" value={pagamento.valorFinal} onChange={(e) => handleDescontoInput(e.target.value, 'final')} />
                  </div>
                </div>
              </div>
              <div className="space-y-4">
